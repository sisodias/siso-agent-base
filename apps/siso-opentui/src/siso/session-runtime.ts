...
export type SisoOpenTuiToolEvent = {
...
export type SisoOpenTuiPromptCallbacks = {
  onAssistantDelta?: (delta: string) => void;
  onToolEvent?: (event: SisoOpenTuiToolEvent) => void;
  onEvent?: (event: AgentSessionEvent) => void;
  onDone?: () => void;
  onError?: (error: Error) => void;
};

export type SisoOpenTuiRuntime = {
  sendPrompt: (text: string, callbacks?: SisoOpenTuiPromptCallbacks) => Promise<void>;
  cancel: () => Promise<void>;
  dispose: () => Promise<void>;
  getSession: () => Promise<AgentSession>;
};

export type SisoOpenTuiRuntimeOptions = {
  cwd?: string;
  agentDir?: string;
  model?: string;
  tools?: string[];
...
export async function createSisoOpenTuiRuntime(options: SisoOpenTuiRuntimeOptions = {}): Promise<SisoOpenTuiRuntime> {
  applySisoEnvironment(options);

  let session: AgentSession | undefined;
  let unsubscribe: (() => void) | undefined;
  let callbacks: SisoOpenTuiPromptCallbacks | undefined;

  async function getSession() {
    if (session) return session;

    const result = options.createSession ? await options.createSession(createSessionOptions(options)) : await createRealSession(options);
...
async function createRealSession(options: SisoOpenTuiRuntimeOptions) {
  const cwd = options.cwd ?? process.env.SISO_OPENTUI_CWD ?? process.cwd();
...
function createSessionOptions(options: SisoOpenTuiRuntimeOptions): CreateAgentSessionOptions {
  return {
    cwd: options.cwd ?? process.env.SISO_OPENTUI_CWD ?? process.cwd(),
...
function dispatchRuntimeEvent(event: AgentSessionEvent, callbacks: SisoOpenTuiPromptCallbacks | undefined) {
  if (!callbacks) return;
...
function applySisoEnvironment(options: SisoOpenTuiRuntimeOptions) {
  loadSecrets();
