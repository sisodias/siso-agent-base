...
if (!fs.existsSync(runsDir)) fail("flight-runs directory missing; run npm run record:doctor-readiness-flight first");
const files = fs.readdirSync(runsDir).filter((file) => file.endsWith(".json") && file !== "index.json" && file !== "doctor-readiness-latest.json").sort();
if (files.length === 0) fail("no flight run traces found");
const traces = files.map((file) => ({ file, trace: JSON.parse(fs.readFileSync(path.join(runsDir, file), "utf8")) }));
